...
class BenchDragon:
    ...
    def visualize_with_collisions(self):
        ...
        plt.title(f'Dragon Collision Detection (t={self.moment}s)\n'
        ...
        plt.savefig(f'./output/Q2/collision_visualization-{self.moment}s.png',
...
def find_collision_start_time():
    ...
    for t in range(start_time, max_time):
        ...
        if has_collision:
            ...
            return t
    print(f"在 {start_time}s 到 {max_time}s 范围内未发现碰撞")
    return None


def save_collision_data(collision_time):
    chart = {}
    BenchDragon_collision = BenchDragon(collision_time)
    chart[f"横坐标x (m)"], chart[f"纵坐标y (m)"] = BenchDragon_collision.get_moment_pos()
    chart[f"速度 (m/s)"] = BenchDragon_collision.get_moment_velo()
    chart = pd.DataFrame(chart)
    with pd.ExcelWriter("./output/Q2/result2.xlsx", engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
        chart.to_excel(writer, sheet_name='Sheet1')


def main():
    collision_start = find_collision_start_time()
    if collision_start:
        save_collision_data(collision_start)
    else:
        print("在检查范围内未发现碰撞")
